    ...
    const KhachHang = () => {
        ...
        const handleDelete = async () => {
            if (selectedCustomers.length > 0) {
                try {
                    const response = await axios.delete(`http://localhost:8000/api/customer/deleteCustomer/${selectedCustomers[0]}`);

                if (response.status === 200) {
                    const updatedCustomers = customers.filter(customer => customer.C_ID !== selectedCustomers[0]);
        ...
        <div className="d-flex justify-content-end my-3">
                        ...
                        <button className="btn btn-danger mr-2" id="btnXoa" onClick={showDeleteConfirmation }>Xóa</button>
                        <button className="btn btn-warning" id="btnSua" onClick={handleShowInfo}>Sửa</button>
                        {showConfirmation && (
        <div className="delete-confirmation">
            <p>Bạn có chắc chắn muốn xóa không?</p>
            <div className="delete-btn">
            <button className="btn btn-primary mr-2 btn-danger delete" onClick={handleDelete}>
                CÓ
            </button>
            <button className="btn btn-secondary delete" onClick={hideDeleteConfirmation}>
                Không
            </button>
            </div>
        </div>
